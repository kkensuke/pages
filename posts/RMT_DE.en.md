...
:::example{title="Column: The Brilliance of RMT: Universality"}
...
Looking at the formula in Theorem 3, when the regularization $\lambda \to 0$ and the number of parameters approaches the number of samples ($p = N_{\mathrm{s}}$), the effective degrees of freedom $\eta_\kappa$ approaches $1$.
...
The result of Theorem 3 possesses not only theoretical beauty but also **extremely high practical value**.
